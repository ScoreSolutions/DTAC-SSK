...
Namespace Config
    Public Class CPUConfigENG
        ...
        Public Sub GetConfigFromXML(ByVal CPUXMLFile As String)
            If File.Exists(CPUXMLFile) = True Then
                Dim xml As New XmlDocument
                xml.Load(CPUXMLFile)

                Dim CPUNode As Xml.XmlNodeList = xml.GetElementsByTagName("CPU")
                If CPUNode.Item(0).InnerXml <> "" Then
                    If CPUNode.Item(0).SelectSingleNode("ShopName") IsNot Nothing Then
                        _ShopName = CPUNode.Item(0).SelectSingleNode("ShopName").InnerText
                    End If
                    _ServerName = CPUNode.Item(0).SelectSingleNode("ServerName").InnerText
                    _IPAddress = CPUNode.Item(0).SelectSingleNode("IPAddress").InnerText
                    _RepeateCheck = CPUNode.Item(0).SelectSingleNode("RepeateCheck").InnerText
                    _ConfigType = CPUNode.Item(0).SelectSingleNode("ConfigType").InnerText
                    _AlarmType = CPUNode.Item(0).SelectSingleNode("AlarmType").InnerText
                    ...
                    For Each asNode As Xml.XmlNode In AlarmSeverityNode
                        ...
                        End If
                    Next
                    _IntervalMinute = CPUNode.Item(0).SelectSingleNode("IntervalMinute").InnerText
                    _Active = CPUNode.Item(0).SelectSingleNode("Active").InnerText
                    _CreateDate = CPUNode.Item(0).SelectSingleNode("CreateDate").InnerText
        ...
        Public Function SaveCPUConfig(ByVal RepeateCheck As Integer, ByVal MinorSeverity As AlarmSeverity, ByVal MajorSeverity As AlarmSeverity, ByVal CriticalSeverity As AlarmSeverity, ByVal IntervalMinute As Int16, ByVal Active As String, ByVal IsShopInstall As String) As XDocument
            Return SaveConfig(RepeateCheck, "CPU", MinorSeverity, MajorSeverity, CriticalSeverity, IntervalMinute, Active, IsShopInstall)
        End Function

        Public Function SaveCPUConfigWeb(ByVal ShopName As String, ByVal ServerName As String, ByVal IPAddress As String, ByVal RepeateCheck As Integer, ByVal MinorSeverity As AlarmSeverity, ByVal MajorSeverity As AlarmSeverity, ByVal CriticalSeverity As AlarmSeverity, ByVal IntervalMinute As Int16, ByVal Active As String, ByVal IsShopInstall As String) As XDocument
            Return SaveConfigWeb(ShopName, ServerName, IPAddress, RepeateCheck, "CPU", MinorSeverity, MajorSeverity, CriticalSeverity, IntervalMinute, Active, IsShopInstall)
        End Function
    End Class
End Namespace
